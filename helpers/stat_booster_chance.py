...
def get_stat_booster_chance(
    yield_level_score: int, grades: typing.List[int], cultivation_tier: int, blessing_of_the_land_event=False
) -> int:
    stat_booster_chance = _get_yield_level_stat_booster_coefficient(yield_level_score)
    grade_chance = sum(
        [(g - 1) * 5 for g in grades]
    )  # TODO: Serenes forest's formula is vague here. Not sure if they mean the sum, or the lowest/highest grade.
